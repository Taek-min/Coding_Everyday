...
boolean isPlayingThisSecond[MAX_LENGTH];
...
int main()
{
    ...
    //// 메모리 효율면에서 조금 아쉬운 점이 있지만 되게 창의력 있는 로직인듯 굳 :2
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < L; j++)
        {
            isPlayingThisSecond[cnt + j] = 1;
        }
    ...
    //// >> D초마다 알람느낌으로? check하게 짜본건데 두번돌리는건 의미가 없긴한듯
    while (isPlayingThisSecond[sec])
        sec += D;
