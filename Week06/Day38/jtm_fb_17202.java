...
public class Main { 	////도움받고 가요 ~ 개인적으로 제일 깔끔 
	public static void main(String args[]) {
		...
		String mixPhoneNum = ""; //// = 양옆에 띄어쓰기 넣는게 국룰(자바 코딩 컨벤션에서 권장하는 사항을 국룰로 줄여 말하겠음)
		for (int i = 0; i < 8; i++) {
			mixPhoneNum += firstPhone.charAt(i) - '0' ;
			mixPhoneNum += secendPhone.charAt(i) - '0' ;
		}
		printCompatibility(mixPhoneNum);
	}
	
	//// 잘은 모르는데 of 없는게 문법상 맞지않나?
	//// 오 이거 신박하네
	static void printCompatibility(String compatibility) { //함수 사용하는 모습 좋습니다. :2
		while(compatibility.length()!=2) { //// while이랑 ( 사이에 띄어쓰기 하나 넣는게 국룰, != 양옆에 띄어쓰기 넣는게 국룰
			String combined = ""; //// 변수명은 동사로 ㄴㄴ combined 등 추천
			for(int i = 0; i < compatibility.length() - 1; i++) { //// for이랑 ( 사이에 띄어쓰기 하나 넣는게 국룰, 세미콜론 뒤에 띄어쓰기 하나 넣는게 국룰
				combined += (compatibility.charAt(i) + compatibility.charAt(i+1)-'0'*2)%10;
			}
