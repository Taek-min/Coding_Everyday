...
public class Main {
	public static void main(String[] args) {
		...
		resetLieDwarfs(hatNum, sumHat);
		printRealDwarf(shatNum);//함수 사용하는 모습 좋습니다^^
		sc.close();//잘배워갑니다^^ // 이게 뭐람 // 찾아보세용~
	}
	
	//// 코드를 처음 보는 사람이 findLieDwarf를 읽으면 'LieDwarf를 찾아서 반환하는 함수겠구나' 할겁니다
	//// 근데 함수가 하는 일은 그게 아니쥬? 이름 다시 지어주십쇼 :2
	static void resetLieDwarfs(int[] hatNum, int sumHat) {
	...
	static void printRealDwarfs(int[] hatNum) {
		for(int i = 0; i < hatNum.length; i++) {
			if(hatNum[i] != 0) {
